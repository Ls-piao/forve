<template>
  <nav ref="navbar"  :style="{ '--defaultTheme': defaultTheme}" :class="'jp-navbar  jp-navbar--' + navbarLayoutType ">
    <div class="jp-navbar__header">
      ...
      <h1 class="jp-navbar__brand" @click="$router.push({ name: 'home' })">
<!--        <a class="jp-navbar__brand-lg" href="javascript:;"> <img :src="logoName" height="126px" width="40px"/> {{modulesName}}</a>-->
        <a class="jp-navbar__brand-lg" href="javascript:;" style="margin-left: 16px;"> <img :src="logoName" height="36px" width="236px"/></a>
<!--        <a class="jp-navbar__brand-mini" href="javascript:;">-->
<!--          <img :src="logo" height="40px" width="40px"/>-->
<!--        </a>-->
      </h1>
    ...
    <div class="jp-navbar__body clearfix" style="overflow:hidden">
      ...
      <el-menu
        ...
        <el-menu-item class="hide-sm" title="返回首页">
          <template slot="title">
            <el-button type="primary" icon="el-icon-s-home" @click="redirectIndex"></el-button>
          </template>
        </el-menu-item>
        <el-menu-item class="hide-sm">
          <template slot="title">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      modulesName () {
        // return this.$store.state.config.modulesName
        return localStorage.getItem('modulesName')
      },
      logoName () {
        // return this.$store.state.config.modulesName
        return localStorage.getItem('logoName')
      },
      defaultLayout () {
    ...
    created () {
      ...
      this.allMenuList = JSON.parse(sessionStorage.getItem('someMenuList') || '[]')
      if (this.defaultLayout === 'top') {
        if (this.allMenuList.length !== 0) {
          ...
          this.$store.commit('common/updateLeftMenuList', '')
