<template>
  <div class="page">
    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button type="text"  icon="el-icon-view" size="small" @click="xmjl(scope.row)">项目交流</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <WtxxList  ref="WtxxList" ></WtxxList>
  </div>
</template>

<script>
  import XmjbxxForm from './XmForm'
  import WtxxList from './WtxxList'
...
export default {
    ...
    methods: {
      ...
      xmjl (row) {
        this.$refs.WtxxList.xmid = row.id
        this.$refs.WtxxList.refreshList()
        this.$refs.WtxxList.visible = true
      },
      // 获取数据列表
      refreshList () {
        this.loading = true
