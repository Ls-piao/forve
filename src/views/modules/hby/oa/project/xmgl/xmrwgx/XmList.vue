    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button type="text" v-if="'4' ===scope.row.xmzt"  icon="el-icon-view" size="small" @click="rwbg(scope.row)">任务变更</el-button>
          <el-button type="text" v-if="'4' ===scope.row.xmzt && '1' == scope.row.rwStatus"  icon="el-icon-edit" size="small" @click="rwbgfb(scope.row)">任务发布</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
    ...
    methods: {
      ...
      rwbgfb (row) {
        this.$confirm('请确定存在任务变更，且所有任务已变更完成?', '提示', {
