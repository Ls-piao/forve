<template>
  <div class="page">
      ...
      <el-row>
        <el-button type="primary" size="small" icon="el-icon-plus" @click="start()">新建</el-button>
    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button  type="text" v-if="scope.row.xmzt === '2' || scope.row.xmzt === '3'"  color="red"  size="small" @click="todo(scope.row)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
    ...
    methods: {
      start () {
        // 读取流程表单
        let tabTitle = `发起流程【新建项目】`
        let processTitle = `${this.userName} 在 ${this.moment(new Date()).format('YYYY-MM-DD HH:mm')} 发起了 [新的项目]`
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          if (data.success) {
            this.$router.push({
              ...
              query: {procDefId: 'Process_1629266787018:7:2b147988-017b-11ec-b5dc-f83441b94772', procDefKey: 'Process_1629266787018', status: 'start', title: tabTitle, formType: data.flow.formType, formUrl: data.flow.formUrl, formTitle: processTitle}
            })
          }
        })
      },
      ...
      todo (row) {
        ...
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          procInsId: row.procInsId,
          procDefId: 'Process_1629266787018:7:2b147988-017b-11ec-b5dc-f83441b94772',
          procDefKey: 'Process_1629266787018',
          status: 'todo'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
