...
<div class=" bg-white">
  ...
  <FullCalendar defaultView="dayGridMonth" locale="zh-cn" :header="{
      ...
      <MyCalendarForm ref="myCalendarForm" @refreshDataList="refreshList"></MyCalendarForm>
</div>
</template>
<script>
  ...
  export default {
    ...
    methods: { // 选择月份
      ...
      handleEventClick (info) {
        this.$refs.myCalendarForm.init('view', info.event.id, info.event.extendedProps.gxjb)
      },
      ...
      refreshList () {
        this.$http.get('/myCalendar/findGxrcList?title=' + this.searchForm.title).then(({data}) => {
