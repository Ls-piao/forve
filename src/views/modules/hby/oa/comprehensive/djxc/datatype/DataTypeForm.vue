  ...
  <div>
    <el-dialog
      ...
      <el-form size="small" :model="inputForm" ref="inputForm" v-loading="loading"
        ...
        <el-row :gutter="15">
          <el-col :span="12">
            <el-form-item label="父级名称" prop="parent.id" :rules="[]">
              <SelectTree
                ref="parent"
                :props="{
                          value: 'id',             // ID字段名
          ...
          <el-col :span="24">
            <el-form-item label="类型封面" prop="coverImg" :rules="[{required: true, message:'类型封面必须上传', trigger:'blur'}]">
              <el-upload ref="coverImg"
                         v-if="visible"
                         list-type="picture-card"
                         accept=".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF"
                         :action="`${this.$http.BASE_URL}/sys/file/webupload/upload?uploadPath=/dataType/dataType`"
          ...
          <el-col :span="12">
            <el-form-item label="排序" prop="sort" :rules="sortRule">
              <el-input v-model="inputForm.sort" :maxlength="3" placeholder="请填写排序"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  data () {
    let validationValue = (rule, value, callback) => {
      if (value < 0) {
    ...
    return {
      ...
      sortRule: [
        ...
        {validator: validationValue, trigger: 'blur'}
      ]
    }
  },
  components: {
      ...
      this.$nextTick(() => {
        ...
        if (method === 'edit' || method === 'view') { // 修改或者查看
          ...
          this.$http({
            ...
            this.inputForm.coverImg.split('|').forEach((item) => {
              if (item.trim().length > 0) {
                this.coverImgArra.push({name: decodeURIComponent(item.substring(item.lastIndexOf('/') + 1)), url: item})
              }
            })
            this.loading = false
          })
