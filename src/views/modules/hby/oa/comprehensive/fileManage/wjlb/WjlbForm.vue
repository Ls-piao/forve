  ...
  <el-dialog
    ...
    <el-form size="small" :model="inputForm" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
      ...
      <el-row  :gutter="15">
        <el-col :span="24">
            <el-form-item label="父级编号" prop="parent.id"
                ...
                <SelectTree
                      ...
                      v-if="visible"
                      :url="`/wjgl/wjgl/wjlb/treeData?extId=${inputForm.id}`"
        ...
        <el-col :span="24">
            <el-form-item label="文件类别名称" prop="name"
                :rules="[
                  {required: true, message:'文件类别名称不能为空', trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.name" placeholder="请填写文件类别名称"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="24">
      ...
      init (method, obj) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/wjgl/wjgl/wjlb/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
              this.inputForm = this.recover(this.inputForm, data.wjlb)
              this.loading = false
            })
          }
        })
