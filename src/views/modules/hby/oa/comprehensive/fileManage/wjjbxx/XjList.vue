    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button  type="text" v-if="scope.row.state === '2' "  color="red"  size="small" @click="todo(scope.row)">编辑</el-button>
<!--          <el-button type="text" v-if="scope.row.state === '0' || scope.row.state === '1'" size="small" @click="urge(scope.row)">催办</el-button>
          <el-button type="text" v-if="scope.row.state === '0'" size="small" @click="revoke(scope.row)">撤销</el-button>
          <el-button  type="text" v-if="scope.row.state === '2' "  color="red"  size="small" @click="restart(scope.row)">编辑</el-button>-->
<!--          <el-dropdown  size="small" style=" margin-left: 10px;">
            <el-button type="text" size="small">
              更多<i class="el-icon-arrow-down el-icon&#45;&#45;right"></i>
            </el-button>
            <el-dropdown-menu slot="dropdown" >
              <el-dropdown-item v-if="scope.row.state === '0' || scope.row.state === '1'" ><el-button type="text"  size="small" @click="urge1(scope.row)">催办</el-button></el-dropdown-item>
              <el-dropdown-item v-if="scope.row.state === '0'"><el-button type="text"  size="small" @click="revoke(scope.row)">撤销</el-button></el-dropdown-item>
              <el-dropdown-item v-if="scope.row.state === '2' "><el-button  type="text" color="red"  size="small" @click="restart(scope.row)">编辑</el-button></el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>-->
        </template>
      </el-table-column>
    ...
    // 排序
        ...
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          if (data.success) {
            this.$router.push({
              ...
              query: {status: 'start', title: title, formTitle: processTitle, ...pick(data.flow, 'formType', 'formUrl', 'procDefKey', 'taskDefKey', 'procInsId', 'procDefId', 'taskId', 'status', 'title', 'businessId')}
            })
          }
        })
      },
      todo (row) {
        ...
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          procInsId: row.procInsId,
          procDefId: 'Process_1629186347998:19:528fbe7c-017b-11ec-b5dc-f83441b94772',
          procDefKey: 'Process_1629186347998',
          status: 'todo'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
      ...
      revoke (row) {
        this.$confirm(`确定要撤销该流程吗?`, '提示', {
          ...
          this.$http.post('/flowable/process/revokeProcIns', {'id': row.procInsId}).then(({data}) => {
            if (data && data.success) {
              this.$http.post('/filemanage/wjjbxx/zhglWjglBasWjglJbxx/update', {'id': row.procInsId}).then(({data}) => {
                if (data && data.success) {
