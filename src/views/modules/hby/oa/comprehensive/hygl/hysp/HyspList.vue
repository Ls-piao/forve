<template>
  <div class="page">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template  slot-scope="scope">
            ...
            <el-button v-if="hasPermission('hygl:hygl:edit') && scope.row.sqzt === '2'" type="text" icon="el-icon-edit" size="small" @click="edit(scope.row.id)">审批</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <HyspForm  ref="hyspForm" @refreshDataList="refreshList"></HyspForm>
  </div>
</template>

<script>
import HyspForm from './HyspForm'
export default {
  data () {
    return {
      searchForm: {
  ...
  methods: {
    ...
    edit (id) {
      ...
      this.$refs.hyspForm.init('edit', id)
    },
    // 查看
    view (id) {
      this.$refs.hyspForm.init('view', id)
    },
    // 删除
    del (id) {
