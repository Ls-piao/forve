<template>
  <div class="page">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template  slot-scope="scope">
            ...
            <el-button v-if="hasPermission('hygl:hygl:edit') && scope.row.sqzt === '6' && scope.row.sfqyhyqd == '1'" type="text" icon="el-icon-date" size="small" @click="hyqd(scope.row.id)">签到</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <WdhyForm ref="wdhyForm" @refreshDataList="refreshList"></WdhyForm>
    <HyqdForm ref="hyqdForm" @refreshDataList="refreshList"></HyqdForm>
  </div>
</template>

<script>
import WdhyForm from './WdhyForm'
import HyqdForm from '../hyqd/HyqdForm'
export default {
  data () {
    return {
      searchForm: {
  ...
  methods: {
    ...
    // 查看
    view (id) {
      this.$refs.wdhyForm.init('view', id)
    },
    resetSearch () {
    ...
    // 会议签到
    hyqd (hyid) {
      this.$refs.hyqdForm.init('add', hyid)
    }
  }
}
</script>
