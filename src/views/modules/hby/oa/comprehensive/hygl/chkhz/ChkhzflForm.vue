    ...
    <el-dialog
      ...
      <el-form size="small" :model="inputForm" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
        ...
        <el-row  :gutter="15">
          ...
          <!--                :rules="[-->
          <!--                 ]">-->
          <!--              <el-input v-model="inputForm.flag" placeholder="请填写分类标识"     ></el-input>-->
          <!--           </el-form-item>-->
          <!--        </el-col>-->
          <el-col :span="12">
            <el-form-item label="分类说明" prop="content"
              ...
              <el-input v-model="inputForm.content" placeholder="请填写分类说明"     ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="父级编号" prop="parent.id"
              ...
              <SelectTree
                ...
                v-if="visible"
                :url="`/zxsq/flb/xcBgzxLjfl/treeData?extId=${inputForm.id}&flag=9`"
    ...
    init (method, obj) {
      ...
      this.$nextTick(() => {
        ...
        if (method === 'edit' || method === 'view') { // 修改或者查看
          this.loading = true
          this.$http({
            url: `/zxsq/flb/xcBgzxLjfl/queryById?id=${this.inputForm.id}`,
            method: 'get'
          }).then(({data}) => {
            this.inputForm = this.recover(this.inputForm, data.xcBgzxLjfl)
            this.loading = false
          })
        }
      })
