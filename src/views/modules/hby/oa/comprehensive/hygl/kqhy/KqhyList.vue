<template>
  <div class="page">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template  slot-scope="scope">
            ...
            <el-button v-if="hasPermission('hygl:hygl:edit') && scope.row.sqzt === '3'" type="text" icon="el-icon-edit" size="small" @click="startMeeting(scope.row.id)">开启会议</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <KqhyForm  ref="kqhyForm" @refreshDataList="refreshList"></KqhyForm>
  </div>
</template>

<script>
import KqhyForm from './KqhyForm'
export default {
  data () {
    return {
      searchForm: {
  ...
  methods: {
    ...
    // 查看
    view (id) {
      this.$refs.kqhyForm.init('view', id)
    },
    // 开启会议
    startMeeting (id) {
      this.$confirm(`确定执行开启会议操作吗?`, '提示', {
