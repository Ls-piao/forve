        ...
        <el-col :span="12">
          <el-form-item label="父级编号" prop="parent.id"
            ...
            <SelectTree
              ...
              v-if="visible"
              :url="`/ypwh/articles/treeData?extId=${inputForm.id}`"
      ...
      init (method, obj) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/ypwh/articles/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
              this.inputForm = this.recover(this.inputForm, data.articles)
              this.loading = false
            })
          }
        })
