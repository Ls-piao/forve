        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            ...
            this.$http({
              ...
              this.inputForm.images.split('|').forEach((item) => {
                if (item.trim().length > 0) {
                  this.imagesArra.push({name: decodeURIComponent(item.substring(item.lastIndexOf('/') + 1)), url: item})
                }
              })
              this.inputForm.txm.split('|').forEach((item) => {
                if (item.trim().length > 0) {
                  this.txmArra.push({name: decodeURIComponent(item.substring(item.lastIndexOf('/') + 1)), url: item})
                }
              })
