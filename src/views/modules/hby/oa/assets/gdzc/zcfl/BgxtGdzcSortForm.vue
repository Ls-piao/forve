      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="12">
            <el-form-item label="类别编号" prop="code"
                :rules="[
                  {required: true, message:'资产类别编号不能为空', trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.code" placeholder="请填写资产类别编号"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item label="父级编号" prop="parent.id"
                ...
                <SelectTree
                      ...
                      v-if="visible"
                      :url="`/zcfl/bgxtGdzcSort/treeData?extId=${inputForm.id}`"
      ...
      init (method, obj) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/zcfl/bgxtGdzcSort/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
              this.inputForm = this.recover(this.inputForm, data.bgxtGdzcSort)
              this.loading = false
            })
          }
        })
