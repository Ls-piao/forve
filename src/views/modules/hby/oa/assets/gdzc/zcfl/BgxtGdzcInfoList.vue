...
<!--                  <span>
    ...
    <div class="jp-common-layout-center jp-flex-main">
      ...
      <el-row>
        <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:add')" type="primary" size="small" icon="el-icon-plus" @click="add()">添加资产</el-button>
...
<!--            <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:import')" type="default" size="small" icon="el-icon-upload2" title="导入" @click="isImportCollapse = !isImportCollapse"></el-button>-->
    ...
    <el-table-column
      ...
      <el-table-column
        ...
        label="操作">
        <template  slot-scope="scope">
          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:sh')  && scope.row.state === '5'" type="text"  icon="el-icon-edit-outline" size="small" @click="shenhe(scope.row.id)">审核</el-button>
          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:view')" type="text" icon="el-icon-view" size="small" @click="view(scope.row.id)">查看</el-button>
<!--          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:jl')" type="text" icon="el-icon-view" size="small" @click="jl(scope.row.id)">借领</el-button>-->
         <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:edit')"  type="text" icon="el-icon-edit" size="small" @click="edit(scope.row.id)">修改</el-button>
<!--        <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:del') && scope.row.state == '5'"  type="text"  icon="el-icon-delete" size="small" @click="del(scope.row.id)">删除</el-button>-->
...
<!--          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:wxwc') && scope.row.state == '3' " type="text"  icon="el-icon-circle-check" size="small" @click="wxwc(scope.row.id)" title="维修完成"></el-button>-->
          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:view')" type="text"  icon="el-icon-s-data" size="small" @click="oper(scope.row.id)">轨迹</el-button>
          <el-button v-if="hasPermission('zcfl:bgxtGdzcInfo:view')" type="text"  icon="el-icon-more" size="small" @click="lz(scope.row)">流转</el-button>
    ...
    <BgxtGdzcInfoForm  ref="bgxtGdzcInfoForm" @refreshDataList="refreshList"></BgxtGdzcInfoForm>
      <BgxtGdzcEditForm  ref="bgxtGdzcEditForm" @refreshDataList="refreshList"></BgxtGdzcEditForm>
...
<script>
  ...
  import BgxtGdzcEditForm from './BgxtGdzcEditForm'
...
export default {
    ...
    methods: {
      ...
      // 修改
      editTreeNode (data) {
        this.$refs.bgxtGdzcEditForm.init('edit', {id: data.id, parent: {id: '', name: ''}})
      },
      lz (row) {
        this.$refs.bgxtGdzcRecord.refreshList(row.name + '记录', row.id)
      },
      delTreeNode (data) {
    ...
    // 排序
      ...
      edit (id) {
        ...
        this.$refs.bgxtGdzcEditForm.init('edit', id)
      },
      // 查看
      view (id) {
