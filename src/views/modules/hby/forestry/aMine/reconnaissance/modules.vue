      ...
      <div class="listbar">
        ...
        >
         <el-upload
          class="uploadBtn"
          :action="
            `${this.$http.BASE_URL}/hby/ziyuankanchaqukuai/ziyuankanchaquK/import`
          "
...
import addDialog from './addModules'
import viewDialog from './viewModules'
export default {
  name: '',
  components: {
    addDialog,
  ...
  methods: {
    initData () {
      return this.$http({
        url: '/hby/ziyuankanchaqukuai/ziyuankanchaquK/list',
    ...
    exportData () {
      this.$http({
        method: 'get',
        url: '/hby/ziyuankanchaqukuai/ziyuankanchaquK/export',
        responseType: 'blob'
      })
        .then(response => {
          if (!response) {
    ...
    dels (items) {
      ...
      this.$http
        .delete('/hby/ziyuankanchaqukuai/ziyuankanchaquK/delete', {
          params: { ids }
        })
        .then(({ data }) => {
          if (data.code === 200) {
