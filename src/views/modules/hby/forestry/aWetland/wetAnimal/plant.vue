  ...
  <div class="container">
    <div class="page-header">
      <el-form class="searchbox" :inline="true">
        <el-form-item>
          ...
          <el-input v-model="outParams.biaoshi" size="mini"></el-input>
        </el-form-item>
        <el-form-item>
          <template #label> 湿地斑块编码： </template>
          <el-input v-model="outParams.shidibankuai" size="mini"></el-input>
        </el-form-item>
      </el-form>
    ...
    <div class="tableBox">
      <div class="listbar">
        ...
        <el-upload
          class="uploadBtn"
          :action="`${this.$http.BASE_URL}/hby/shidizhiwu/shidizhiwu/import`"
...
import addDialog from './addPlant'
import viewDialog from './viewPlant'
export default {
  name: '',
  components: {
    addDialog,
  ...
  data () {
    return {
      ...
      outParams: { biaoshi: '', shidibankuai: '' },
      headers: {
  ...
  methods: {
    initData () {
      return this.$http({
        url: '/hby/shidizhiwu/shidizhiwu/list',
    ...
    exportData () {
      this.$http({
        method: 'get',
        url: '/hby/shidizhiwu/shidizhiwu/export',
        responseType: 'blob'
      }).then(response => {
        if (!response) {
          return
    ...
    dels (items) {
      ...
      this.$http
        .delete('/hby/shidizhiwu/shidizhiwu/delete', {
          params: { ids }
        })
        .then(({ data }) => {
          if (data.code === 200) {
