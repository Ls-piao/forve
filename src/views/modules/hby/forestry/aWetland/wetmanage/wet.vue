  ...
  <div class="container">
    ...
    <div class="tableBox">
      <div class="listbar">
       ...
       <el-upload
          class="uploadBtn"
          :action="`${this.$http.BASE_URL}/hby/shidiqugl/shidiqugl/import`"
...
<script>
import addDialog from './addWet'
import { cloneDeep } from 'lodash'
import viewDialog from './viewWet'
export default {
  name: '',
  components: {
    addDialog,
  ...
  methods: { initData () {
    return this.$http({
      url: '/hby/shidiqugl/shidiqugl/list',
    ...
    exportData () {
      this.$http({
        method: 'get',
        url: '/hby/shidiqugl/shidiqugl/export',
        responseType: 'blob'
      }).then(response => {
        if (!response) {
          return
    ...
    dels (items) {
      ...
      this.$http
        .delete('/hby/shidiqugl/shidiqugl/delete', {
          params: { ids }
        })
        .then(({ data }) => {
          if (data.code === 200) {
