    ...
    <div class="tableBox">
      <div class="listbar">
        ...
        >
         <el-upload
          class="uploadBtn"
          :action="`${this.$http.BASE_URL}/hby/shidibaohu/shidibaohu/import`"
...
import addDialog from './addProtect'
import viewDialog from './viewProtect'
export default {
  name: '',
  components: {
    addDialog,
  ...
  methods: {
    initData () {
      return this.$http({
        url: '/hby/shidibaohu/shidibaohu/list',
    ...
    exportData () {
      this.$http({
        method: 'get',
        url: '/hby/shidibaohu/shidibaohu/export',
        responseType: 'blob'
      }).then(response => {
        if (!response) {
          return
    ...
    dels (items) {
      ...
      this.$http
        .delete('/hby/shidibaohu/shidibaohu/delete', {
          params: { ids }
        })
        .then(({ data }) => {
          if (data.code === 200) {
