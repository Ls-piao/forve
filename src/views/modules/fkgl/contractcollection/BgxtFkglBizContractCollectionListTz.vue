...
<script>
  ...
  export default {
    ...
    methods: {
      start () {
        // 读取流程表单
        let tabTitle = `发起流程【合同收款】`
        let processTitle = `${this.userName} 在 ${this.moment(new Date()).format('YYYY-MM-DD HH:mm')} 发起了 [合同收款]`
        this.$http.get('/flowable/task/getTaskDef', {params: {
          procDefId: 'fkglhtsk:9:518da011-ff29-11eb-97e7-7e7635e53918',
          status: 'start'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
              path: '/flowable/task/TaskForm',
              query: {procDefId: 'fkglhtsk:9:518da011-ff29-11eb-97e7-7e7635e53918', procDefKey: 'fkglhtsk', status: 'start', title: tabTitle, formType: data.flow.formType, formUrl: data.flow.formUrl, formTitle: processTitle}
            })
          }
        })
      },
