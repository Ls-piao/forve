...
<script>
  ...
  export default {
    ...
    methods: {
      start () {
        // 读取流程表单
        let tabTitle = `发起流程【开票申请】`
        let processTitle = `${this.userName} 在 ${this.moment(new Date()).format('YYYY-MM-DD HH:mm')} 发起了 [开票申请]`
        this.$http.get('/flowable/task/getTaskDef', {params: {
          procDefId: 'fkglkpsq:3:f74b9675-ff29-11eb-97e7-7e7635e53918',
          status: 'start'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
              path: '/flowable/task/TaskForm',
              query: {procDefId: 'fkglkpsq:3:f74b9675-ff29-11eb-97e7-7e7635e53918', procDefKey: 'fkglkpsq', status: 'start', title: tabTitle, formType: data.flow.formType, formUrl: data.flow.formUrl, formTitle: processTitle}
            })
          }
        })
      },
