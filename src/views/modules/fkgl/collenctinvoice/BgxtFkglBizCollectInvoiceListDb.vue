    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="todo(scope.row)">重新办理</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      start () {
        // 读取流程表单
        let tabTitle = `发起流程【收票记录】`
        let processTitle = `${this.userName} 在 ${this.moment(new Date()).format('YYYY-MM-DD HH:mm')} 发起了 [收票记录]`
        this.$http.get('/flowable/task/getTaskDef', {params: {
          procDefId: 'fkglspjl:3:84457b49-ff29-11eb-97e7-7e7635e53918',
          status: 'start'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
              path: '/flowable/task/TaskForm',
              query: {procDefId: 'fkglspjl:3:84457b49-ff29-11eb-97e7-7e7635e53918', procDefKey: 'fkglspjl', status: 'start', title: tabTitle, formType: data.flow.formType, formUrl: data.flow.formUrl, formTitle: processTitle}
            })
          }
        })
      },
      ...
      refreshList () {
        this.loading = true
        this.$http({
          url: '/fkgl/collenctinvoice/bgxtFkglBizCollectInvoice/listBySh',
          method: 'get',
          params: {
            'pageNo': this.pageNo,
