<template>
    ...
    <el-table-column
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.reimbursementDate">
            {{ scope.row.reimbursementDate.substring(0,10) }}
          </span>
        </template>
      </el-table-column>
    <el-table-column
    ...
    <el-table-column
      ...
      <el-table-column
        ...
        <template  slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="dayin(scope.row)">打印</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <BgxtFkglBizReimbursementInfoDetail  ref="BgxtFkglBizReimbursementInfoDetail" @refreshDataList="refreshList"></BgxtFkglBizReimbursementInfoDetail>
  </div>
</template>

<script>
  import pick from 'lodash.pick'
  import BgxtFkglBizReimbursementInfoDetail from './BgxtFkglBizReimbursementInfoDetail'
  export default {
    data () {
      return {
        fileUrlArra: [],
    ...
    components: {
      BgxtFkglBizReimbursementInfoDetail
    },
    activated () {
      this.refreshList()
    },
    ...
    methods: {
      dayin (row) {
        this.$refs.BgxtFkglBizReimbursementInfoDetail.init(row.id)
      },
      todo (row) {
        var key = 'fkgl_cxfq'
        var title1 = '重新办理'
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          procDefId: 'fkglclfbx:4:7a4df726-00d0-11ec-a0b1-7e7635e53918',
          procDefKey: 'fkglclfbx',
          status: 'todo'
        }}).then(({data}) => {
          if (data.success) {
            this.$router.push({
      ...
      start () {
        ...
        this.$http.get('/flowable/task/getTaskDef', {params: {
          ...
          if (data.success) {
            this.$router.push({
              path: '/flowable/task/TaskForm',
              query: {procDefId: 'fkglclfbx:4:7a4df726-00d0-11ec-a0b1-7e7635e53918', procDefKey: 'fkglclfbx', status: 'start', title: tabTitle, formType: data.flow.formType, formUrl: data.flow.formUrl, formTitle: processTitle}
            })
          }
        })
      },
