  ...
  <el-dialog
    ...
    <el-form :model="inputForm" size="small" ref="inputForm" :class="method==='view'?'readonly':''"  :disabled="method==='view'" @keyup.enter.native="doSubmit()"
      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="12">
            <el-form-item label="出差补助" prop="affiliatedUnit"
                :rules="[
                  {validator: validator.isNumber, trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.affiliatedUnit" placeholder="请填写出差补助"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
            <el-form-item label="住宿费" prop="travelAllowance"
                :rules="[
                 ]">
              <el-input v-model="inputForm.travelAllowance" placeholder="请填写住宿费"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
  ...
  export default {
    ...
    methods: {
      init (method, obj) {
        this.method = method
        if (method === 'add') {
          this.title = `新建办公系统费控管理报销-差旅费报销内容`
        } else if (method === 'edit') {
          this.title = '修改办公系统费控管理报销-差旅费报销内容'
        } else if (method === 'view') {
          this.title = '查看办公系统费控管理报销-差旅费报销内容'
        }
        this.visible = true
        this.$nextTick(() => {
